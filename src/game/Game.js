...
export class Game {
	...
	update( dt ) {
		...
		if ( inp.hit( 'KeyR' ) && can && ! this.fight ) {
			...
			this.toast( rod.equipped ? ( inp.mobile ? 'Rod out · hold Cast, then release' : 'Rod out · hold left mouse to cast' ) : 'Rod away', 1600 );
		...
		const lmb = inp.buttonDown( 0 ), rmb = inp.buttonDown( 2 );
